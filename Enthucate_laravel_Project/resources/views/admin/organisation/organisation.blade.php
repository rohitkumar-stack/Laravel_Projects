                                        ...
                                        if(!empty($results)){
                                            ...
                                            if(isset($results['organisations']) && $results['organisations'] != ''){
                                                foreach ($results['organisations'] as $key => $result) { 
                                                    ...
                                                    <td>
                                                        <div class="d-flex">
                                                        @if(isset($org_edit->meta_value) && $org_edit->meta_value == 'on')
                                                            <a href="{{ url(Auth::User()->organisation_url.'/admin/edit-organisation/'.$result->id ) }}" class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fa fa-pencil"></i></a>
                                                            @endif
...
<div class="modal fade" id="markdelete" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
         ...
         <form class="form" method="POST" id="organisationdelete" action="{{ url(Auth::User()->organisation_url.'/admin/delete-organisation/') }}">
            @csrf

...
@section('title', 'grade List - Enthucate')
@section('content')
<!--**********************************
	Content body start
***********************************-->
                                    ...
                                    if(!empty($results)){
                                        foreach ($results as $key => $result) { 
                                           $sno = $key+1;
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{ url(Auth::User()->organisation_url.'/admin/edit-grade/'.$result->id ) }}" class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fa fa-pencil"></i></a>
                                                    <a href="javascript:void(0)" class="btn btn-danger shadow btn-xs sharp markgradedelete" data-id="{{$result->id}}" data-toggle="modal" data-target="#deletegrade" title="Delete grade" data-placement="right" ><i class="fa fa-trash"></i></a>
                                                </div>
                                            </td>
                                            </tr>
                                            <?php
...
<div class="modal fade" id="deletegrade" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
         ...
         <form class="form" method="POST" id="gradedelete" action="{{ url(Auth::User()->organisation_url.'/admin/delete-grade/') }}">
            @csrf

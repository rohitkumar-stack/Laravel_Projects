...
class VerifyaccountController extends Controller
{
    ...
    public function TeacherVerify(Request $request, $id, $role)
    {
        // return redirect('/dashboard');
        $userid = base64_decode($id);
        $role_id = base64_decode($role);
        ...
        return view('verifyaccount.teacherverify',['id'=>$id,'user'=>$user,'role_id'=>$role_id,'schools'=> $schools]);
    }
    ...
    public function CreateTeacherAccount(Request $request)
    {
        ...
        $input['grade']=$request->grade;
        $input['class']=$request->class;
         // foreach ($request->school as $keys => $school) {
        foreach ($input as $key => $value) {
        ...
        //foreach ($request->school as $keys => $school) {
            ...
            // echo '<pre>';
                ...
                $input_data['role_id']= $user->role_id;
                // $input_data['school_id']= $request->school[$keys];
                // $input_data['grade_id']= $request->grade[$keys];
                // $input_data['class_id']= $request->class[$keys]; 
                if(is_array($value)) {
                   $values = implode(",",$value); 
                } 
                else{
